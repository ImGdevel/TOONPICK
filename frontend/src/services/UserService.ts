...
class UserService {
  ...
  public async addFavoriteWebtoon(webtoonId: number): Promise<UserResponse> {
    try {
      await api.post(`/api/users/favorites/${webtoonId}`, null, { authRequired: true });
      return { success: true };
    } catch (error) {
  ...
  public async removeFavoriteWebtoon(webtoonId: number): Promise<UserResponse> {
    try {
      await api.delete(`/api/users/favorites/${webtoonId}`, { authRequired: true });
      return { success: true };
    } catch (error) {
  ...
  public async isFavoriteWebtoon(webtoonId: number): Promise<UserResponse<boolean>> {
    try {
      const response = await api.get<boolean>(`/api/users/favorites/${webtoonId}/is-favorite`, { authRequired: true });
      return { success: true, data: response };
    } catch (error) {
  ...
  public async getBookmarks(): Promise<UserResponse> {
    try {
      const response = await api.get('/api/users/bookmarks', { authRequired: true });
      return { success: true, data: response };
    } catch (error) {
