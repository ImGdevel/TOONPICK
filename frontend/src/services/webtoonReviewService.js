...
// 특정 리뷰 가져오기
export const getWebtoonReviewById = async (reviewId) => {
  try {
    const response = await api.get(`/api/webtoon/reviews/${reviewId}`);
    return { success: true, data: response.data };
  } catch (error) {
...
// 리뷰 삭제
export const deleteWebtoonReview = async (reviewId) => {
  try {
    await api.delete(`/api/webtoon/reviews/${reviewId}`);
    return { success: true };
  } catch (error) {
