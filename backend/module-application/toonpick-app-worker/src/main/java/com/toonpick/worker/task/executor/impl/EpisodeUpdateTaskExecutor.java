...
/**
 * 에피소드 업데이트 작업 실행자
 * 웹툰 에피소드 업데이트 작업을 실행하는 클래스
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class EpisodeUpdateTaskExecutor implements TaskExecutor {
    ...
    public void execute(TaskContext context) {
        if (!validate(context)) {
            log.error("에피소드 업데이트 작업 검증 실패: {}", context);
            throw new IllegalArgumentException("Invalid task context");
        }
        
        log.info("에피소드 업데이트 작업 시작: {}", context.getTaskId());
        
        try {
            // 에피소드 업데이트 실행 전략 가져오기
            TaskExecutionStrategy strategy = strategyFactory.getTaskExecutionStrategy(TaskType.WEBTOON_EPISODE_UPDATE);
            
            // 전략 실행
            strategy.execute(context);
            
            log.info("에피소드 업데이트 작업 완료: {}", context.getTaskId());
            
        } catch (Exception e) {
            log.error("에피소드 업데이트 작업 실패: {}", context.getTaskId(), e);
            throw e;
        }
    }
    ...
    public String getExecutorName() {
        return "EpisodeUpdateTaskExecutor";
    }
} 